...
class M_kirim extends CI_Model {
	...
	function kirim_budget_simakda(){
      ...
      ini_set("max_execution_time","600");
		...
		kodeprogram,kodekegiatan FROM kua.belanjarincian ORDER BY kodeurusan,kodesuburusan,rtrim(kodeorganisasi),rtrim(kodebidang),
	...
	function kirim_budget_simpatda_wisnu(){

		$dbsimpatda=$this->load->database('simpatdasql', TRUE);
		$dbsimakda=$this->load->database('simakda', TRUE);
		// $dbsimpatda->query("select * from trdsts");
		$sql = "SELECT a.kd_sts,a.kd_rek,a.nilai FROM TRDSTS a INNER JOIN
		TRHSTS b ON a.kd_sts = b.kd_sts WHERE (YEAR(b.tanggal) = '2019')";
		$query1 = $dbsimpatda->query($sql);
		foreach($query1->result_array() as $resulte2) {
			$kd_sts 			= $resulte2['kd_sts'];
			$kd_rek 			= $resulte2['kd_rek'];
			$nilai 				= $resulte2['nilai'];

		$dbsimakda->query("insert into trdsts(no_bukti,kd_rek5,nilai) 
							values('$kd_sts','$kd_rek','$nilai') ");
			   ...
			   $sql2 = "select  distinct  kb.kodeakun,kb.kodekelompok,kb.kodejenis,kb.kodeobjek,kb.koderincianobjek,ks.kodesumberdana  
	    		...
	    		foreach($query2->result_array() as $resulte2) { 
					...
					$cno_trdrka 		= $ckd_skpd.'.'.$ckd_kegiatan.'.'.$ckd_rek5 ;
					 ...
					 $sql3 = "select kb.urai as uraian,kb.volume as volume1,kb.volume2,kb.volume3,kb.jmlvolume as tvolume, kb.satuan as satuan1,kb.satuan2,kb.satuan3,kb.jmlsatuan,kb.hargasatuan as harga1,
						kb.pagu as total, kb.paguubah as total_ubah from kua.belanjarincian kb inner join kua.belanjasub ks on kb.kodeurusan=ks.kodeurusan and kb.kodesuburusan=ks.kodesuburusan and kb.kodeorganisasi=ks.kodeorganisasi and 
						kb.kodebidang=ks.kodebidang and kb.kodeprogram=ks.kodeprogram and kb.kodekegiatan = ks.kodekegiatan and kb.id=ks.id 
						and kb.kodesub=ks.kodesub Where kb.kodeurusan='$ckodeurusan' And kb.kodesuburusan='$ckodesuburusan' And 
						kb.kodeorganisasi='$ckodeorganisasi' And kb.kodebidang='$xkodebidang' And kb.kodeprogram='$ckodeprogram' And 
						kb.kodekegiatan='$ckodekegiatan' And kb.kodeakun = '$ckodeakun' And kb.kodekelompok = '$ckodekelompok' And 
	    			...
	    			foreach($query3->result_array() as $resulte3) { 
						...
						$dbsimakda->query("insert into trdpo(kd_kegiatan,kd_rek5,no_trdrka,uraian,volume1,volume2,volume3,tvolume,satuan1,satuan2,satuan3,harga1,total,total_ubah,sumber) 
							values('$ckd_kegiatan','$ckd_rek5','$cno_trdrka','$curai','$cvolume1','$cvolume2','cvolume3','$cjmlvolume','$csatuan1','$csatuan2','$csatuan3','$chargasatuan','$ctotal','$ctotal_ubah','$cnmdana') ");
